				       ...
				       int              *logical_widths)
{
  ...
  item.offset = 0;
  item.length = length;
  item.num_chars = g_utf8_strlen(text, length);

  item.analysis.shape_engine = NULL;
  item.analysis.lang_engine = NULL;
  item.analysis.font = NULL;
  item.analysis.level = embedding_level;
  item.analysis.gravity = PANGO_GRAVITY_AUTO;
  item.analysis.flags = 0;
  item.analysis.script = PANGO_SCRIPT_UNKNOWN;
  item.analysis.language = NULL;
  item.analysis.extra_attrs = NULL;
